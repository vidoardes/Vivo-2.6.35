...
#define VIVO_ACPU_MIN_UV_MV 750U
#ifdef CONFIG_XTREME_MODE
#define VIVO_ACPU_MAX_UV_MV 1500U
#else
#define VIVO_ACPU_MAX_UV_MV 1350U
#endif
...
static struct cpufreq_frequency_table freq_table[] = {
	...
	{ 14, 1420800 },
	{ 15, 1497600 },
#ifndef CONFIG_XTREME_MODE
	{ 16, CPUFREQ_TABLE_END },
#else //CONFIG_XTREME_MODE
	{ 17, 1574400 },
	{ 18, 1651200 },
	{ 19, 1728000 },
	{ 20, 1804800 },
...
static unsigned int acpuclk_get_current_vdd(void)
{
	...
	for (vdd_mv = VIVO_ACPU_MIN_UV_MV; vdd_mv <= VIVO_ACPU_MAX_UV_MV; vdd_mv += V_STEP)
		if (VDD_RAW(vdd_mv) == vdd_raw)
			break;

	if (vdd_mv > VIVO_ACPU_MAX_UV_MV)
		return 0;
...
static int acpuclk_update_freq_tbl(unsigned int acpu_khz, unsigned int acpu_vdd)
{
	...
	if (acpu_vdd > VIVO_ACPU_MAX_UV_MV || acpu_vdd < VIVO_ACPU_MIN_UV_MV) {
...
void acpuclk_set_vdd(unsigned int khz, int vdd)
{
	...
	{
		if (khz == 0)
			new_vdd = min(max((acpu_freq_tbl[i].vdd_mv + vdd), VIVO_ACPU_MIN_UV_MV), VIVO_ACPU_MAX_UV_MV);
		else if (acpu_freq_tbl[i].acpu_clk_khz == khz)
			new_vdd = min(max((unsigned int)vdd, VIVO_ACPU_MIN_UV_MV), VIVO_ACPU_MAX_UV_MV);
		else continue;
